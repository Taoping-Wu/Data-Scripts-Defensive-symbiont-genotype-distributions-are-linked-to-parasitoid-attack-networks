...
### Plant
#############
dataset <- read.csv("Data_18_Plant_genus_aphid_31_species_Fig.1.csv", header=T, row.names = 1)

normalized_dataset <- apply(dataset, 1, function(row) row / sum(row))
normalized_dataset <- t(normalized_dataset)
df <- as.data.frame(normalized_dataset)
...
#write.csv(df_long, file = "Long_plant_box_plot_20240424.csv")
#df_long$Genus <- sub("\\..*$", "", df_long$Species)
#df_long <- read.csv("Long_Para_box_plot_45sp csv")


#### cluster methods comparison####

# Assuming 'population_data' is your data frame/matrix with rows as areas and columns as species populations
...
### Parasitoids
#############
dataset <- read.csv("Data_17_Parasitoid_genus_aphid_22_species_Fig.1.csv", header=T, row.names = 1)
normalized_dataset <- apply(dataset, 1, function(row) row / sum(row))
normalized_dataset <- t(normalized_dataset)
df <- as.data.frame(normalized_dataset)
